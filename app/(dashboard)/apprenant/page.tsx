// app/(dashboard)/apprenant/page.tsx - Dispatcher pour apprenant
import { redirect } from 'next/navigation';
import { supabaseServer } from '@/lib/supabase/server';
import { getUserOrganizations } from '@/lib/org-server';

export default async function ApprenantIndex() {
  ...
  try {
    ...
    if (organizations.length === 1) {
      ...
      redirect(`/apprenant/${org.slug}/dashboard`);
    }

    // Plusieurs organisations - rediriger vers le picker
    redirect('/apprenant/select-org');
  } catch (error) {
    console.error('Error in apprenant dispatcher:', error);
    redirect('/login/apprenant');
  }
}