// app/(dashboard)/formateur/page.tsx - Dispatcher pour formateur
import { redirect } from 'next/navigation';
import { supabaseServer } from '@/lib/supabase/server';
import { getUserOrganizations } from '@/lib/org-server';

export default async function FormateurIndex() {
  ...
  try {
    ...
    if (organizations.length === 1) {
      ...
      redirect(`/formateur/${org.slug}/dashboard`);
    }

    // Plusieurs organisations - rediriger vers le picker
    redirect('/formateur/select-org');
  } catch (error) {
    console.error('Error in formateur dispatcher:', error);
    redirect('/login/formateur');
  }
}