// app/(dashboard)/tuteur/page.tsx - Dispatcher pour tuteur
import { redirect } from 'next/navigation';
import { supabaseServer } from '@/lib/supabase/server';
import { getUserOrganizations } from '@/lib/org-server';

export default async function TuteurIndex() {
  ...
  try {
    ...
    if (organizations.length === 1) {
      ...
      redirect(`/tuteur/${org.slug}/dashboard`);
    }

    // Plusieurs organisations - rediriger vers le picker
    redirect('/tuteur/select-org');
  } catch (error) {
    console.error('Error in tuteur dispatcher:', error);
    redirect('/login/tuteur');
  }
}