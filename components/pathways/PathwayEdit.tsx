...
export default function PathwayEdit({ org, id }: PathwayEditProps) {
  ...
  const { data: pathwayResponse, isLoading, error } = usePathway(id);
  const updatePathway = useUpdatePathway();
  const updateItems = useUpdatePathwayItems(id);
  const updateAssignments = useUpdatePathwayAssignments(id);
  const { success, error: showError } = useToast();

  // Charger les listes pour les modals
  const { data: formationsResponse } = useFormationsList(org);
  const { data: resourcesResponse } = useResourcesList(org);
  const { data: testsResponse } = useTestsList(org);
  const { data: learnersResponse } = useLearnersList(org);
  const { data: groupsResponse } = useGroupsList(org);
