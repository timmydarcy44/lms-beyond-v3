...
interface AdminTestDetailPageProps {
  params: Promise<{
    slug: string;
  }>;
}

export default async function AdminTestDetailPage({ params }: AdminTestDetailPageProps) {
  const { slug } = await params;
  const category: LearnerCategory = "tests";
            ...
            <div className="max-w-3xl space-y-6 text-white">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-4">
                <Button asChild className="rounded-full bg-gradient-to-r from-white via-amber-100 to-orange-400 px-6 py-2 text-sm font-semibold text-black shadow-[0_16px_50px_rgba(249,115,22,0.35)] hover:from-white hover:via-white">
                  <Link href={playHref}>Passer le test</Link>
                </Button>
              </div>
            </div>
          </div>
