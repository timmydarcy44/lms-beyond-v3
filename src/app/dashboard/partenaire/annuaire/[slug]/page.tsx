...
function PartenaireDetailContent() {
  ...
  const gradientStyle = useMemo(() => {
    ...
    if (slug === "normandie-energie") {
      return {
        background:
          "linear-gradient(135deg, #1a1a2e 0%, #16213e 40%, #0f3460 70%, #8B1A2B 100%)",
      };
    }
    return {
    ...
    <div className="min-h-screen bg-[#080d14] text-white">
        ...
        <div className="static mt-4 px-4 lg:absolute lg:bottom-0 lg:left-8 lg:-translate-y-1/2 lg:px-0">
          <div className="flex min-w-[320px] items-center gap-4 rounded-2xl border border-white/20 bg-[#111827] p-5 shadow-2xl">
            {partenaire.logo ? (
              ...
              <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-[#8B1A2B] text-lg font-black text-white lg:text-2xl">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <span className="rounded-full bg-[#8B1A2B]/20 px-2 py-0.5 text-xs text-[#8B1A2B]">
                  Partenaire Football Club Rochelais
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="mt-10 border-b border-white/10 bg-[#0d1520] px-4 py-4 lg:px-8">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 text-sm text-white/60">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button className="rounded-full bg-[#8B1A2B] px-4 py-2 text-sm font-semibold text-white">🤝 Proposer un deal</button>
          </div>
        </div>
      </div>
      ...
      {activeTab === "about" && (
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 py-4 lg:grid-cols-[1fr_300px] lg:px-8 lg:py-8">
          <div>
            <div className="border-l-4 border-[#8B1A2B] pl-6 text-lg font-light italic text-white/80 lg:text-2xl">
              “Être partenaire de Football Club Rochelais, c'est s'ancrer dans le tissu local...”
            ...
            <div className="mt-6 space-y-4 text-white/60 leading-relaxed">
              ...
              <p>
                ...
                Football Club Rochelais, un club qui partage nos valeurs de proximité et d'engagement territorial.
              </p>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 lg:grid-cols-2">
              {partenaire.services.map((service) => (
                <div
                  key={service.titre}
                  className="rounded-xl border border-transparent bg-[#111827] p-4 transition-all hover:border-[#8B1A2B]/40"
                >
                  ...
                  <div className="mt-2 text-sm font-bold text-[#8B1A2B]">{service.prix}</div>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-2xl bg-[#111827] p-5">
              ...
              <div className="mt-3 space-y-3">
                ...
                <textarea
                  ...
                  className="w-full rounded-xl bg-white/10 px-4 py-2 text-sm text-white"
                />
                <button className="w-full rounded-xl bg-[#8B1A2B] px-4 py-2 text-sm font-semibold text-white">Envoyer →</button>
              </div>
            </div>
            <div className="rounded-2xl bg-[#111827] p-5">
