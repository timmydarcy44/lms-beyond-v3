...
export default function TutorMissionsIndexPage() {
    ...
    <TuteurShell tutorName={tutorName} navBadges={{ missions: data?.kpis.pendingMissionActions }}>
      <div className="bg-[#f5f5f7] min-h-screen px-6 py-10">
        <div className="max-w-5xl mx-auto space-y-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Missions à valider</h1>
            <p className="text-sm text-gray-500">Choisissez un alternant pour voir et valider ses missions.</p>
          </div>

          {loading ? <p className="text-sm text-gray-500">Chargement…</p> : null}

          {!loading && assignments.length === 0 ? (
            <p className="text-sm text-gray-600">Aucun alternant rattaché.</p>
          ) : null}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {assignments.map((alternant) => (
              <Link
                ...
                className="bg-white rounded-2xl shadow-sm p-6 hover:shadow-md transition-shadow duration-200 block"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{alternant.ecole ?? "Organisme"}</p>
                    <p className="text-sm text-gray-600 mt-1">
                      {alternant.missionsAValider} mission{alternant.missionsAValider > 1 ? "s" : ""} à traiter
                    </p>
                  </div>
                </div>
              </Link>
