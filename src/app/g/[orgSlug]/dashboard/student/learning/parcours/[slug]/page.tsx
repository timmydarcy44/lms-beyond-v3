...
export default async function GalaxyLearnerParcoursDetailPage(props: {
  params: Promise<{ orgSlug: string; slug: string }>;
}) {
  const { orgSlug, slug } = await props.params;

  const data = await getApprenantDashboardData(orgSlug);
  const parcoursCard = data.parcours.find((item) => item.slug === slug);
  if (!parcoursCard) notFound();

  const pathId = parcoursCard.id;
  const pathContent = pathId ? await getLearnerPathDetail(pathId) : null;

  const steps = Array.isArray((pathContent as any)?.steps) ? (((pathContent as any).steps as any[]) ?? []) : [];

  const courseById = new Map((pathContent?.courses ?? []).map((c: any) => [String(c.id), c]));
  const testById = new Map((pathContent?.tests ?? []).map((t: any) => [String(t.id), t]));
  const resourceById = new Map((pathContent?.resources ?? []).map((r: any) => [String(r.id), r]));
    ...
    for (const s of steps) {
      ...
      if (contentKind === "course") {
        ...
        blocks.push({
          ...
          href: `/g/${encodeURIComponent(orgSlug)}/catalog/formations/${encodeURIComponent(slugOrId)}`,
          meta: "Formation",
      ...
      if (contentKind === "test") {
        ...
        blocks.push({
          ...
          href: `/g/${encodeURIComponent(orgSlug)}/catalog/tests/${encodeURIComponent(slugOrId)}`,
          meta: "Évaluation",
        });
        continue;
      }
      ...
      if (contentKind === "resource") {
        ...
        blocks.push({
          ...
          href: `/g/${encodeURIComponent(orgSlug)}/catalog/ressources/${encodeURIComponent(slugOrId)}`,
  ...
  const formationItems =
    pathContent && pathContent.courses.length > 0
      ? buildColumnItems(
          pathContent.courses.map((c) => ({
            ...(c as any),
            href: `/g/${encodeURIComponent(orgSlug)}/catalog/formations/${encodeURIComponent(String((c as any).slug ?? ""))}`,
  ...
  const testItems =
    pathContent && pathContent.tests.length > 0
      ? buildColumnItems(
          pathContent.tests.map((t) => ({
            ...(t as any),
            href: `/g/${encodeURIComponent(orgSlug)}/catalog/tests/${encodeURIComponent(String((t as any).slug ?? ""))}`,
            meta: String((t as any).description ?? "").trim() || "Évaluation",
            progress: 0,
          })) as any,
          "tests",
          "Test",
  ...
  const resourceItems =
    pathContent && pathContent.resources.length > 0
      ? buildColumnItems(
          pathContent.resources.map((r) => ({
            ...(r as any),
            href: `/g/${encodeURIComponent(orgSlug)}/catalog/ressources/${encodeURIComponent(String((r as any).slug ?? ""))}`,
            meta: (r as any).type ? `Ressource ${(r as any).type}` : "Ressource complémentaire",
            progress: 0,
          })) as any,
          "ressources",
          "Ressource",
    ...
    if (user?.id) {
      ...
      if (testItems.length > 0 && ids.length > 0) {
        let res: any = await supabase
          .from("test_attempts")
          .select("id, test_id, percentage, total_score, max_score, completed_at, created_at")
          .eq("user_id", user.id)
          .in("test_id", ids)
        ...
        if (res?.error?.code === "42703" || res?.error?.code === "PGRST204") {
          res = await supabase
            .from("test_attempts")
            .select("id, test_id, percentage, completed_at, created_at")
            .eq("user_id", user.id)
            .in("test_id", ids)
  ...
  const bestPercentForTestId = (testId: string) => {
    ...
    const quizPercent = typeof quiz?.score === "number" && Number.isFinite(quiz.score) ? quiz.score : null;
  ...
  const timelineWithLocks = (() => {
    ...
    for (const s of steps) {
      ...
      if (type === "trigger") {
        ...
        const aiMinRaw = typeof (s as any)?.trigger_ai_min_score === "number" ? (s as any).trigger_ai_min_score : Number((s as any)?.trigger_ai_min_score);
        ...
        if (linkedTestId) {
          ...
          quiz = {
            ...
            href: `/g/${encodeURIComponent(orgSlug)}/catalog/tests/${encodeURIComponent(String(t?.slug ?? linkedTestId))}?testId=${encodeURIComponent(linkedTestId)}`,
          };
        }
  ...
  const heroCover = (pathContent as any)?.cover_image || (parcoursCard as any).image || null;
  ...
  const heroIsVideo = typeof heroCover === "string" && isVideoLike(String(heroCover));

  const timelineHasCourses = timelineWithLocks.some((b: any) => b?.kind === "course");

  return (
    <LearningSessionTracker contentType="path" contentId={pathId} showIndicator={false}>
      <DashboardShell
        title={parcoursCard.title}
        breadcrumbs={[
          { label: "Dashboard", href: `/g/${encodeURIComponent(orgSlug)}/dashboard/student/learning/formations` },
          { label: "Parcours", href: `/g/${encodeURIComponent(orgSlug)}/dashboard/student/learning/parcours` },
        ...
        <div className="space-y-10">
          ...
          <section id="composition" className="space-y-8">
            ...
            <div className="space-y-10">
                  ...
                  {timelineWithLocks.map((block: any, idx: number) => {
                          ...
                          <div className="flex min-w-0 flex-1 flex-col gap-3">
                            ...
                            >
                              <div className="inline-flex min-w-0 flex-wrap items-center gap-2">
                                ...
                                {!quiz && String((block as any).label ?? "").includes("Formation") ? (
                              ...
                              <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                                ...
                                {learnerTrigger ? (
                                  <LearnerTriggerCTA trigger={learnerTrigger} satisfied={Boolean((block as any).satisfied)} />
                                ) : null}
                              </div>
                            </div>
                          </div>
                    ...
                    if ((block as any).kind === "course") {
                        ...
                        <>
                          ...
                          <div className="flex flex-1 flex-col gap-3 md:flex-row md:items-center">
                            <div className="relative h-24 w-full overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800 via-slate-900 to-black md:h-20 md:w-44">
                              {item.cover_url ? (
                                isVideoLike(String(item.cover_url)) ? (
                                  <video
                      ...
                      return (
                        ...
                        >
                          {item.locked ? (
                            <div role="group" aria-disabled className="flex min-w-0 flex-wrap gap-4 outline-none">
                              {row}
                            </div>
                          ) : (
                            <Link
                ...
                <details className="group/notes rounded-2xl border border-white/10 bg-white/[0.04] open:border-white/15">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {pathContent?.tests?.map((t: any) => {
                      ...
                      const percent =
                        typeof attempt?.percentage === "number" ? Math.round(attempt.percentage) : null;
                      const scoreLabel =
                        percent != null
                          ? `${percent}%`
                          : attempt?.total_score != null && attempt?.max_score != null
                            ...
                            : "Pas encore de note";
                      return (
                        <Card key={`test-note-${t.id}`} className="overflow-hidden rounded-3xl border border-white/10 bg-white/5">
              ...
              {formationItems.length > 0 && !timelineHasCourses && renderContentSection("Formations", "formations", formationItems)}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => (
          <Link key={`${title}-${item.title}`} href={item.href} className="group">
            <Card
              className={cn(
              ...
              <div className="relative h-48 w-full overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <h4 className="line-clamp-2 text-lg font-semibold text-white transition group-hover:text-white/90">
                        {item.title}
                      </h4>
                      <p className="mt-1 text-xs uppercase tracking-[0.3em] text-white/60">{item.meta}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <CardContent className="space-y-3 p-5">
                <div className="space-y-2">
                  ...
                  <div className="relative h-2 overflow-hidden rounded-full bg-white/10">
                    <div
                      className={cn("absolute inset-y-0 left-0 rounded-full transition-all duration-500", `bg-gradient-to-r ${progressGradient[accent]}`)}
