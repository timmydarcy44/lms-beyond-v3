...
type TasksBannerProps = {
  roleFilter?: "learner" | "instructor" | "tutor" | "admin";
  todoHref: string;
};

export function TasksBanner({ roleFilter, todoHref }: TasksBannerProps) {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        let url = "/api/todo-tasks?status=todo,in_progress";
        if (roleFilter) {
        ...
        if (response.ok) {
          ...
          if (roleFilter === "learner") {
            ...
            filteredTasks = data.filter((task: TodoTask) => {
              return !task.task_type || learnerTaskTypes.includes(task.task_type);
            });
          } else if (roleFilter === "instructor") {
            const instructorTaskTypes = ['grading', 'review', 'preparation', 'meeting', 'feedback'];
            filteredTasks = data.filter((task: TodoTask) => {
              return !task.task_type || instructorTaskTypes.includes(task.task_type);
            });
          } else if (roleFilter === "tutor") {
            const tutorTaskTypes = ['followup', 'meeting', 'report', 'evaluation'];
            filteredTasks = data.filter((task: TodoTask) => {
              return !task.task_type || tutorTaskTypes.includes(task.task_type);
            });
          } else if (roleFilter === "admin") {
            const adminTaskTypes = ['approval', 'review', 'report', 'configuration'];
            filteredTasks = data.filter((task: TodoTask) => {
              return !task.task_type || adminTaskTypes.includes(task.task_type);
            });
          }
          ...
          console.warn("[tasks-banner] Failed to load tasks:", response.status);
        }
      } catch (error) {
        console.error("[tasks-banner] Error loading tasks:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <Card className="border-white/10 bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-md">
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          >
            <Link href={todoHref} className="flex items-center gap-1.5">
              Voir tout
