...
import { assignContentToGroup } from "@/app/dashboard/student/studio/apprenants/actions";
import { cn } from "@/lib/utils";
import type { AssignableContent } from "@/lib/queries/formateur";
import type { FormateurGroup } from "@/lib/queries/formateur";

type GroupAssignmentModalProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  group: FormateurGroup;
...
export function GroupAssignmentModal({
  ...
  const handleAssign = () => {
    ...
    startTransition(async () => {
      try {
        const result = await assignContentToGroup(group.id, {
        ...
        if (result.success) {
          toast.success(`${result.count} élément(s) assigné(s) avec succès au groupe "${group.name}"`);
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Assigner du contenu au groupe "{group.name}"
          </DialogTitle>
          <DialogDescription>
            Sélectionnez les formations, parcours, ressources et tests à assigner à tous les membres de ce groupe ({group.members_count} membre{group.members_count > 1 ? "s" : ""}).
          </DialogDescription>
        </DialogHeader>
