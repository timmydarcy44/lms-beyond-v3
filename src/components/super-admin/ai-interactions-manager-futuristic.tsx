...
export function AIInteractionsManagerFuturistic() {
  ...
  return (
    <div className="space-y-8">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 bg-gray-900/50 border border-cyan-500/20 backdrop-blur-sm">
          <TabsTrigger
            value="prompts"
            className="flex items-center gap-2 data-[state=active]:bg-cyan-500/20 data-[state=active]:text-cyan-400 data-[state=active]:border-cyan-500/50"
          >
          ...
          <div className="grid gap-6">
            {AI_FEATURES.map((feature) => {
              ...
              return (
                <Card
                  key={feature.id}
                  className="border-cyan-500/20 bg-gradient-to-br from-gray-900/80 to-black/80 backdrop-blur-sm shadow-[0_0_20px_rgba(0,255,255,0.1)] hover:shadow-[0_0_30px_rgba(0,255,255,0.2)] transition-all"
                >
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <CardTitle className="text-cyan-400 text-xl mb-2">{feature.name}</CardTitle>
                      ...
                      {!isEditing && (
                        <Button
                          ...
                          className="border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 hover:border-cyan-500/50"
                        >
                    ...
                    <div className="flex items-center gap-2 mt-4">
                      <Badge
                        variant="outline"
                        className="border-cyan-500/30 text-cyan-400/80 bg-cyan-500/10"
                      >
                        {feature.endpoint}
                      </Badge>
                      <Badge
                    ...
                    {isEditing ? (
                      <div className="space-y-4">
                        <Textarea
                          ...
                          className="min-h-[300px] bg-black/50 border-cyan-500/30 text-cyan-100 font-mono text-sm focus:border-cyan-500/50 focus:ring-cyan-500/20"
                          placeholder="Entrez le prompt..."
                        />
                        <div className="flex items-center gap-2">
                          <Button
                            onClick={() => handleSave(feature.id)}
                            className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white shadow-[0_0_20px_rgba(0,255,255,0.3)]"
                          >
                      ...
                      <div className="rounded-lg bg-black/50 border border-cyan-500/20 p-4">
                        <pre className="text-sm text-cyan-100/90 font-mono whitespace-pre-wrap overflow-x-auto">
                          {currentPrompt}
                        </pre>
                        {promptsFromDB[feature.id] && (
                          <Badge
                            variant="outline"
                            className="mt-3 border-green-500/30 text-green-400 bg-green-500/10"
                          >
                            Personnalisé
                          </Badge>
                        )}
        ...
        <TabsContent value="history" className="space-y-6 mt-6">
          <Card className="border-cyan-500/20 bg-gradient-to-br from-gray-900/80 to-black/80 backdrop-blur-sm shadow-[0_0_20px_rgba(0,255,255,0.1)]">
            <CardHeader>
            ...
            <CardContent>
              <AIInteractionsHistoryFuturistic />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
...
function AIInteractionsHistoryFuturistic() {
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400 text-sm mt-4">Chargement de l'historique...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <label className="text-sm text-gray-400">Filtrer par fonctionnalité :</label>
        <select
          value={selectedFeature || ""}
          onChange={(e) => setSelectedFeature(e.target.value || null)}
          className="rounded-lg border border-cyan-500/30 bg-black/50 px-3 py-2 text-sm text-cyan-100 focus:border-cyan-500/50 focus:ring-cyan-500/20"
        >
      ...
      {interactions.length === 0 ? (
        <div className="text-center py-12">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-cyan-500/10 border border-cyan-500/20 mb-4">
            <History className="h-8 w-8 text-cyan-400/50" />
          </div>
          <p className="text-gray-400 text-sm">
            Aucune interaction IA enregistrée pour le moment.
          </p>
          <p className="text-gray-500 text-xs mt-2">
            Les interactions seront enregistrées automatiquement lors de l'utilisation des fonctionnalités IA.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {interactions.map((interaction) => (
            <Card
              key={interaction.id}
              className="border-cyan-500/20 bg-gradient-to-br from-gray-900/50 to-black/50 backdrop-blur-sm hover:border-cyan-500/40 transition-all"
            >
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h4 className="text-cyan-400 font-medium">{interaction.feature_name}</h4>
