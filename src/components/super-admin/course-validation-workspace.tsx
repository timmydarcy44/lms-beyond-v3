...
function CourseValidationContent() {
  ...
  const handleEdit = (type: "section" | "chapter", id: string, currentValue: string) => {
    if (type === "section") {
      ...
      <Card className="border-gray-200 bg-white">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-semibold text-gray-900">Validation de la nomenclature</CardTitle>
      ...
      {structure.length === 0 ? (
        <Card className="border-gray-200 bg-white">
          <CardContent className="py-12 text-center">
            <p className="text-gray-600">Aucune structure à valider</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {structure.map((section) => (
            <Card key={section.id} className="border-gray-200 bg-white">
              <CardContent className="p-6">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1">
                    {editingSection === section.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-900 focus:border-blue-500 focus:outline-none"
                          onKeyDown={(e) => {
                    ...
                    {section.description && (
                      <p className="text-sm text-gray-600 mt-1 ml-7">{section.description}</p>
                    )}
                  </div>
                </div>
...
export function CourseValidationWorkspace() {
  return (
