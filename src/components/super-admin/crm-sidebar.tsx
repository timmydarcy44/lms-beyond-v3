...
function isLinkActive(
  ...
  if (isPipelineBtob) {
    return pathname === "/super/crm/pipeline" && typeParam !== "btoc" && !pathname.startsWith("/super/crm/pipeline/prescripteurs");
  }
  if (isPrescripteur) {
  ...
  return (
    <aside className="w-full shrink-0 border-b border-gray-200 bg-gray-50/90 lg:w-56 lg:border-b-0 lg:border-r lg:min-h-[calc(100vh-3rem)]">
      <div className="sticky top-12 p-2 lg:p-4">
        <div className="-mx-1 flex gap-1 overflow-x-auto px-1 pb-1 lg:mx-0 lg:block lg:space-y-4 lg:overflow-visible lg:px-0 lg:pb-0">
          {sections.map((section) => (
            <div key={section.key} className="flex shrink-0 items-center gap-1 lg:block lg:space-y-1">
              <p className="hidden px-3 pb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-gray-400 lg:block">
                {section.label}
              </p>
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={cn("h-4 w-4 shrink-0", isActive && item.featured ? "opacity-100" : "opacity-70")}
                    />
