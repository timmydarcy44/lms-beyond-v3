...
import { fetchLastSignInForUser } from "@/lib/queries/learner-tracking";
import {
...
import type {
  LearnerCoachingActivity,
  LearnerCourseEngagement,
  LearnerDossier,
  LearnerDossierTestAttempt,
  LearnerFlashcardSession,
  LearnerInterviewSession,
  LearnerSkillInsight,
} from "@/lib/queries/learner-dossier-types";
import { formatLearnerDuration } from "@/lib/queries/learner-dossier-types";

// ---------------------------------------------------------------------------
// Helpers (module interne — pas exportés depuis "use server")
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    for (const session of interviewSessions) {
      ...
      coachingActivities.unshift({
        ...
        detail: session.feedback?.summary ?? `${session.userTurnCount} échanges · ${formatLearnerDuration(session.durationSeconds)}`,
