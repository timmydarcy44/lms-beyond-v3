-- Prod: table manquante (PGRST205) — migration 20260605210000
-- Coller dans Supabase → SQL Editor → Run, puis vérifier le SELECT EXISTS ci-dessous.
...
grant all on public.collaborateur_entreprise_consentements to service_role;

-- Vérification (doit retourner true)
select exists (
  select 1 from information_schema.tables
  where table_schema = 'public'
    and table_name = 'collaborateur_entreprise_consentements'
) as table_exists;
